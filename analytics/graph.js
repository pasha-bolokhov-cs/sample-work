...
// Initializes _________ section
graphApp.controller('_________Controller', function ($scope, $rootScope, $http, $q) {
	...
	$scope.refresh = function() {
		load_________Data($rootScope.whatRange(), $scope, $rootScope, $http, $q).then(
	...
	init_________Chart($scope, $rootScope);
	$scope.refresh();
});
...
// Creates the necessary objects for the ********* chart
function init_________Chart($scope, $rootScope) {
...
// Obtain and compile together the data for the ********* chart
function load_________Data(range, $scope, $rootScope, $http, $q) {
	...
	return $q(function(resolve, reject) {
		...
		function(response) {
			...
			*********.addColumn('number', '==========');
			...
			var aver_********* = new google.visualization.DataTable();
			aver_*********.addColumn('date', 'Date');
			aver_*********.addColumn('number', 'Average ==========');
			for (r = 0; r < json_aver_*********.length; r++) {
				aver_*********.addRow([
					new Date(
						json_aver_*********[r].year,
						json_aver_*********[r].month - 1,
						json_aver_*********[r].day
					),
					json_aver_*********[r].number
				]);
			...
			combined_********* = google.visualization.data.join(*********, aver_*********, 'full',
				[ [ 0, 0 ] ], [ 1 ], [ 1 ]);
			resolve(combined_*********);
		},
		function(response) {
